...
interface DashboardLocalGigsContentProps {
  initialStats: LocalGigsStats;
}

export function DashboardLocalGigsContent({ initialStats }: DashboardLocalGigsContentProps) {
  // useState: local toggle for modals, buttons (rule 6 - ephemeral UI state)
  const displayStats = [
    { 
      label: 'Gigs Completed', 
      value: initialStats.gigsCompleted.toString(), 
      icon: MapIcon 
    },
    { 
      label: 'Active Listings', 
      value: initialStats.activeListings.toString(), 
      icon: MapPin 
    },
    { 
      label: 'Total Earnings', 
      value: `KES ${(initialStats.totalEarnings || 0).toLocaleString()}`, 
      icon: DollarSign 
    },
  ];

...
export default function ChatForeignersPage() {
  ...
  const [apiStatus, setApiStatus] = useState<'nvidia' | 'fallback'>('nvidia');
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-screen bg-[#0d0d14] items-center justify-center">
        ...
        <p className="text-zinc-500 mt-3 text-sm">Loading personalities...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0d0d14] text-zinc-100 overflow-y-auto">
      ...
      {/* API Status Badge */}
      {apiStatus === 'fallback' && (
        <div className="mx-4 mb-3 bg-amber-900/20 border border-amber-700/50 rounded-lg px-3 py-2 flex items-center gap-2">
          <div className="w-2 h-2 bg-amber-500 rounded-full animate-pulse" />
          <span className="text-xs text-amber-400">Operating on optimized mode</span>
        </div>
      )}

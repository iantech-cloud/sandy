...
export default function GamingMpesaWaitingPage() {
  ...
  const getStatusConfig = (status: PaymentStatus['status']): StatusConfig => {
    const config: Record<PaymentStatus['status'], StatusConfig> = {
      processing: {
        ...
        description: 'Please check your phone and enter your M-Pesa PIN to complete the gaming deposit.',
        color: 'text-blue-600',
        bgColor: 'bg-blue-50',
      ...
      success: {
        ...
        description: `Your deposit of KES ${amount} has been completed successfully. Funds have been added to your gaming wallet.`,
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div
        className={`max-w-md w-full ${statusConfig.bgColor} ${statusConfig.borderColor} border-2 rounded-2xl p-8 shadow-lg`}
      >
        <div className="text-center">
          ...
          <p className="text-gray-700 mb-6 leading-relaxed">{statusConfig.description}</p>
          ...
          {paymentStatus.status === 'success' && paymentStatus.mpesaReceiptNumber && (
            <div className="bg-green-100 border border-green-300 rounded-lg p-4 mb-6">
              <div className="text-sm text-green-800">
                ...
                <div className="text-xs mt-2 text-green-700">Keep this receipt for your records</div>
              </div>
            </div>
          )}
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-6 p-3 bg-gray-100 rounded-lg">
              <div className="text-xs text-gray-600 text-left space-y-1">
                <div>
                  <strong>MessageReference:</strong> {messageReference}
                </div>
                <div>
                  <strong>Status:</strong> {paymentStatus.status}
                </div>
                <div>
                  <strong>Result Code:</strong> {paymentStatus.resultCode}
                </div>
                <div>

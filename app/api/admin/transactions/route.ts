...
export async function GET(request: NextRequest) {
  try {
    ...
    // ── Legacy filter ───────────────────────────────────────────────────────
    const legacyFilter: any = { target_type: 'user', ...dateFilter };
    ...
    const fetchLegacy = collection !== 'cf'  && !!LegacyTx;
    const fetchCF     = collection !== 'legacy';

    // ── True DB-level pagination ────────────────────────────────────────────
    // Cap each collection at (skip + limit) rows so memory stays bounded.
    // We count both, merge-sort, then slice to the requested page.
    const fetchUpTo = skip + limit;

    const [legacyCount, cfCount, legacyTxns, cfTxns] = await Promise.all([
      fetchLegacy ? LegacyTx!.countDocuments(legacyFilter) : Promise.resolve(0),
      fetchCF     ? (ChatForeignersTransaction as any).countDocuments(cfFilter) : Promise.resolve(0),
      fetchLegacy
        ? LegacyTx!.find(legacyFilter)
            .populate('user_id',              'username email')
            .populate('mpesa_transaction_id', 'mpesa_receipt_number')
            .select('_id amount_cents type description status transaction_code mpesa_transaction_id metadata created_at balance_after_cents source target_type user_id')
            .sort({ created_at: -1 })
            .limit(fetchUpTo)
            .lean()
        : Promise.resolve([]),
    ...
    const normLegacy = (txn: any) => {
      ...
      const coopRef  = txn.transaction_code || 'N/A';
      const mpesaRef = txn.mpesa_transaction_id?.mpesa_receipt_number || meta.mpesaReceiptNumber || 'N/A';
    ...
    const normCF = (txn: any) => ({
      ...
      status:              txn.status || 'N/A',
      description:         txn.description || TYPE_LABELS[txn.type] || 'Chat Foreigners Transaction',
      date:                txn.created_at,
      target_type:         txn.target_type || 'user',
    ...
    const total  = legacyCount + cfCount;

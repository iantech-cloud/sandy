  ...
  return (
    <header className="flex flex-col sm:flex-row justify-between items-center py-4 px-4 md:px-12 bg-surface shadow-sm sticky top-0 z-50 border-b border-border">
      ...
      <nav className="hidden sm:flex space-x-4 sm:space-x-8 items-center" aria-label="Primary navigation">
        ...
        <Link href="/auth/login" className="text-text-muted hover:text-indigo-600 transition-colors">Login</Link>
        <ThemeToggle />
      </nav>
      ...
      >
        ...
        <Link href="/auth/login" className="block py-2 px-3 text-text hover:bg-bg-subtle rounded-lg transition-colors" onClick={toggleMenu}>Login</Link>
      </nav>
    </header>
  );
};

...
export default function UsersAdminPage() {
  const [accesses, setAccesses] = useState<PersonAccess[]>([]);
  ...
  const loadUserAccess = async () => {
    try {
      ...
      if (data.success) {
        // Flatten all accesses from all persons
        const allAccesses = data.data.flatMap((person: any) =>
          (person.userAccess || []).map((access: any) => ({
            ...access,
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">View user person access and engagement metrics</p>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Person Name</th>
            ...
            {accesses.length === 0 ? (
              ...
              accesses.map((access) => (
                <tr key={access._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-900">{access.personName}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {formatDistanceToNow(new Date(access.createdAt || Date.now()), { addSuffix: true })}
                  </td>
                </tr>
              ))
            )}

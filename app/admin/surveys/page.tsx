...
import { Search, AlertCircle, Edit, Trash2, Plus, Eye } from 'lucide-react';
    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600">Loading surveys...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Surveys Management</h1>
          <p className="text-slate-600 mt-1">
            Total: {data?.pagination.total.toLocaleString()} surveys
          </p>
        </div>
      ...
      {data && (
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow p-4 border border-slate-200">
            ...
            <p className="text-2xl font-bold text-green-600 mt-1">{data.stats.active}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4 border border-slate-200">
            <p className="text-slate-600 text-sm">Inactive</p>
            <p className="text-2xl font-bold text-slate-600 mt-1">{data.stats.inactive}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-4 border border-slate-200">
            <p className="text-slate-600 text-sm">Responses</p>
            <p className="text-2xl font-bold text-blue-600 mt-1">{data.stats.totalResponses}</p>
          </div>
        </div>
      )}
                ...
                data.surveys.map((survey) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <p className="text-slate-600 text-xs mt-1">{survey.description}</p>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-slate-600 text-sm">{survey.question_count}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-slate-600 text-sm">{survey.total_responses}</p>
                    </td>
                    <td className="px-6 py-4">
                      <select
                        value={survey.status}
                        onChange={(e) => handleStatusChange(survey._id, e.target.value)}
                        disabled={processingId === survey._id}
      ...
      {data && data.pagination.pages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-slate-600">
            Showing {data.surveys.length} of {data.pagination.total} surveys
          </p>
          <div className="flex gap-2">
            <button
